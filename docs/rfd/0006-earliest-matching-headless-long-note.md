...
**Status:** Accepted (2026-06-27) — 1차(싱글 헤드 없는 롱노트) 구현 완료, §11 참조
...
1차 구현은 **싱글 헤드 없는 롱노트(길이 0 슬라이드 포함)의 keydown 흡수**에 집중한다 — 핵심 문제(슬라이드 직후 탭 흡수)가 여기서 해결된다. 다음은 규칙만 확정하고 실제 코드는 해당 노트가 차트에 등장할 때로 미룬다:
...
미구현(§3.4 규칙만 확정): 헤드 없는 더블 롱노트 2키 흡수, 릴리즈 노트(길이 0 일반) keydown 흡수 — 해당 노트가 차트에 등장할 때 구현.
