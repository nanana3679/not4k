...
**해결됨** (5절 검증 + 6절 케이스로): tie-break(earliest→레인→인덱스), 흡수 윈도우(노트 good window), 흡수 종료(필요 키 수 충족), 같은 시점 롱노트+포인트(=헤드 있는 롱노트).

**남은 질문**:

1. **헤드 없는 더블 롱노트의 2 keydown 흡수**: 두 keydown을 각 키에 귀속시키는 방식과 grace period(키 전환) 상호작용.
2. **릴리즈 노트(길이 0 일반)의 keydown/keyup 분리**: keyup 기반 판정과 분리하면서 keydown은 직후 노트 보호용으로만 흡수하는 범위 — Sonolus의 `disallowEnd` 락아웃 참고.
3. **회귀 검증 대상**: 홀드 중 탭, 홀드 이어잡기, 릴리즈탭, 연결 판정 체인(`.-.-`)이 이 변경으로 깨지지 않는지.

이 질문들은 규칙의 철학(헤드 없는 롱노트가 keydown을 판정 없이 흡수, 필요 키 수 충족 후 종료)이 아니라 윈도우·경계·구현 세부이므로, 후속 구현과 회귀 테스트에서 결정한다.
