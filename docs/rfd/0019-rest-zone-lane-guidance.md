...
- **Phase 2:**
  ...
  4. 에디터 저작 UX — `trillZone` 에디터 경로를 슬라이스로 복제한다:
     ...
     - **(채택 — 배타 시절 미채택을 뒤집음)** 박스 감쌈 픽업 — 배타 축 시절엔 박스가 노트·restZone을 함께 감싸면 배타 규칙과 충돌해 미채택했으나, 공존 전환으로 충돌이 사라져 채택(2026-07-16). `boxEnclosesRestZone`(`boxEnclosesZone` 미러) + `selectionFromBox`의 `restZones` 축 — 박스가 restZone 구간 [beat,endBeat]를 완전히 포함하고 레인 범위 안이면 유닛 픽업, 노트와 함께 감싸면 둘 다 선택.
