...
`ChartData`에 `restZones: RestZone[]` 축을 추가하고 JSON 직렬화에 round-trip시킨다. 전역 이벤트(`ChartEvent`)가 아니라 레인 귀속이라 `trillZone`과 같은 top-level 배열로 둔다. 여러 레인이 동시에 쉴 수 있다(오른손 파킹 = L3·L4에 각각 하나).

### 4-2. 두 가지 기능 — 그리고 오직 이 둘

1. **시각 효과:** 게임에서 해당 레인 구간을 dim해 "가라앉은" 것으로 보인다(§구현 기록의 밴드). 활성 레인 대비 명도 차로 "이 레인 쉼"이 읽힌다.
2. **배치 제약:** 에디터에서 `restZone`이 덮은 레인·구간에 **노트를 놓을 수 없다.** 저작 시점 예방이므로 "휴지 구간인데 노트가 있는" 모순이 애초에 성립하지 않는다(사후 검증이 아니라 제약). 역방향(노트가 있는 자리에 `restZone` 배치)도 배치 제약으로 다룬다.

**판정 불개입:** 런타임 입력·판정·기록·랭킹·실패 조건은 `restZone`과 무관하다(§3-3).

### 4-3. 시각 결과 — 레인 base 밝기 상향(전역)

dim이 보이려면 활성 레인이 완전 검정보다 밝아야 한다(near-black은 더 어둡게 눌러도 안 보임). 따라서 레인 base 톤을 전역으로 올린다 — `restZone`이 없는 차트도 레인이 살짝 밝아지는 시각 변화가 따른다. 수용된 결과다.
...
- **Phase 2 (미구현):** 저작 본체 —
  1. `ChartData.restZones` 타입 + JSON 직렬화 + 검증
  2. 게임 렌더 소스 교체(파생기 삭제 → `chartData.restZones`)
