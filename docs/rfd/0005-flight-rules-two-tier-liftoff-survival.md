# RFD 0005: `flightRule` 2종 재편 — Liftoff / Survival
...
- [`docs/rfd/0003-flight-rules-positioning-newcomer-motivation.md`](0003-flight-rules-positioning-newcomer-motivation.md) — `flightRule` 포지셔닝
- [`docs/context/glossary.md`](../context/glossary.md) — `flightRule` 용어 정의
- [`docs/spec/game-core.md`](../spec/game-core.md) — `flightRule` 스펙
- [`docs/spec/scoring.md`](../spec/scoring.md) — 달성률·랭크와 `flightRule`의 관계
...
RFD 0001은 Play에 `altitude` 기반 `flightRule` 3종(`Takeoff / Ascent / Breakthrough`)을 도입했고,
RFD 0002는 Breakthrough의 Perfect-only 회복을 확정했으며, RFD 0003은 `flightRule`을
...
2. `flightRule`을 별도 축으로 두면 선택 설정이 하나 더 생겨, 입문 유저에게 초기 부담이 될 수 있다.
3. 반대로 `flightRule`을 난이도명에 종속(통합)시키면, 같은 차트에서 도전 강도를 따로 고를 수
   없고 숙련자 정확도 축이 난이도명에 묶인다. 이는 RFD 0001의 "난이도명·Lv.·`flightRule`은
   별도 축" 결정과 RFD 0003의 "숙련자 압박은 Breakthrough 전담" 결정과 충돌한다.

## 3. 결정

**`flightRule`을 2종으로 재편한다: `Liftoff`(관대, 기본값) / `Survival`(더 어려움).**
난이도명(EASY/NORMAL/HARD)과 `flightRule`은 **계속 별도 축으로 유지한다.** 종속·통합하지 않는다.

세부 규칙:

- **Liftoff** — Takeoff와 Ascent를 통합한 관대한 입문 규칙. `altitude` 0에서 시작해 곡 종료 시
  기준 `altitude` 이상이면 클리어되며, 곡 중 게임 오버가 없다. 숙련 유저 기준 사실상 실패하지
  않는 수준으로 관대하게 밸런싱한다(RFD 0003 포지셔닝 유지). **`flightRule`의 기본값이다.**
- **Survival** — Breakthrough를 계승한 숙련자 정확도 규칙. `altitude` 100에서 시작해 `Perfect`로만
  `altitude`를 회복하고, `Great / Good`은 회복하지 않으며, `Bad / Miss`와 빈 레인 입력 Bad는
  `altitude`를 감소시키고, 0 도달 시 실패한다. RFD 0002의 Perfect-only 회복 모델을 그대로 계승한다.
  Liftoff보다 명확히 어렵다.
- **입문 부담 해소** — 기본값을 가장 관대한 Liftoff로 고정하고, Survival은 숙련 유저가
  선택적으로 켠다. 입문 유저는 `flightRule` 설정을 건드릴 필요가 없으므로, 별도 축으로 두어도
  초기 부담이 사실상 발생하지 않는다(progressive disclosure).
- **명칭** — UI와 코드는 영어 명칭 `liftoff` / `survival`을 사용한다.
- **수치** — 정확한 `altitude` 증감량과 Survival의 난이도 정도(얼마나 어려운가)는 경험적으로
...
### B. `flightRule`을 난이도명에 종속(통합)

난이도를 고르면 `flightRule`이 자동 결정되어 별도 선택이 사라진다.
...
관대 규칙을 Liftoff 하나로 통합하고, 숙련 규칙을 Survival로 계승하며, `flightRule`은 별도
...
입문 유저 동기 포지셔닝은 그대로 유지한다. 이 RFD는 `flightRule`의 **종류를 3종에서 2종으로
...
- `docs/context/glossary.md` — `flightRule` 항목을 `Liftoff / Survival` 2종으로 갱신한다.
- `CONTEXT.md`, `src/game/CONTEXT.md` — `flightRule` 용어·정의를 갱신한다.
- `docs/spec/game-core.md` — `flightRule` 종류와 관대 밸런싱 서술을 갱신한다.
- `docs/spec/scoring.md` — `flightRule` 명칭 참조를 갱신한다.
- `docs/research/beatmania.md` — IIDX 참조 모델의 not4k 대응 규칙명을 갱신한다.
- `docs/prd.md`, `README.md` — 기능 목록의 `flightRule` 명칭을 갱신한다.
...
3. `flightRule`의 기본값을 Liftoff로 둔다는 결정 외에, 차트별 추천 `flightRule` 노출 여부.
4. 랭킹과 플레이 기록을 `flightRule`별로 분리할 것인가(RFD 0001 §9 승계).
