# RFD 0003: 비행 규칙의 포지셔닝 — 입문 유저 동기 장치
...
RFD 0001은 Play에 고도 기반 비행 규칙(`Takeoff / Ascent / Breakthrough`)을 도입했고,
RFD 0002는 Breakthrough의 회복 규칙을 확정했다. 그러나 두 문서 모두
**게이지(비행 규칙)를 왜 도입하는가, 누구를 위한 장치인가**를 기록하지 않았다.

RFD 0001의 문제의식은 "비행 규칙의 긴장감"과 "beatmania식 클리어 목표와 생존 압박"
...
비행 규칙 도입의 또 다른 목적인 "약간의 내러티브(비행 콘셉트) 도입"은 RFD 0001의
...
RFD 0001의 Play/Observer 경계, 비행 규칙 명칭, 난이도 축 분리와
RFD 0002의 Breakthrough Perfect-only 회복은 그대로 유지한다.
이 RFD는 비행 규칙의 도입 목적과 Takeoff/Ascent의 밸런싱 방향만 추가로 확정한다.
...
- `docs/context/glossary.md`, `CONTEXT.md`
  - 비행 규칙 항목에 이 RFD 링크를 추가한다.
- 후속 비행 규칙 구현 스펙 (RFD 0001 §8의 `game-core.md`/`scoring.md` 갱신 포함)
...
2. 입문 유저가 실패했을 때 Observer로의 안내 경로를 어떻게 설계하는가
   (RFD 0001 §9 Q4와 연결).
3. 비행 규칙의 기본값은 가장 관대한 Takeoff인가 (RFD 0001 §9 Q5와 연결).
