# RFC 0003: 비행 규칙의 포지셔닝 — 입문 유저 동기 장치
...
RFC 0001은 Play에 고도 기반 비행 규칙(`Takeoff / Ascent / Breakthrough`)을 도입했고,
RFC 0002는 Breakthrough의 회복 규칙을 확정했다. 그러나 두 문서 모두
**게이지(비행 규칙)를 왜 도입하는가, 누구를 위한 장치인가**를 기록하지 않았다.

RFC 0001의 문제의식은 "비행 규칙의 긴장감"과 "beatmania식 클리어 목표와 생존 압박"
측면에 치우쳐 있다. 이 프레이밍만 남으면 밸런싱 단계에서 비행 규칙을
...
비행 규칙의 도입 목적이 기록되지 않으면 다음 위험이 생긴다.

1. 밸런싱 담당(미래 에이전트 포함)이 Takeoff/Ascent를 숙련자 압박 장치로
   해석하여 실패가 잦은 수치로 구현할 수 있다.
2. 실패가 잦아지면 RFC 0001 이전의 핵심 철학 — 가변 손배치 학습을 중도 실패로
   끊지 않는다 — 이 사실상 폐기된다.
...
**not4k의 게이지(비행 규칙)는 리듬게임에 익숙하지 않은 유저에게 동기를 제공하는
장치로 포지셔닝한다.** DJMAX RESPECT V 게이지의 체감과 같은 맥락이다.

비행 규칙 도입의 또 다른 목적인 "약간의 내러티브(비행 콘셉트) 도입"은 RFC 0001의
배경·비주얼 원칙이 담당한다. 이 RFC는 규칙으로서의 게이지가 누구를 위한 장치인지만
확정한다.
...
- **숙련 유저 대상 정확도 압박은 Breakthrough(RFC 0002) 단일 규칙이 전담한다.**
  Takeoff/Ascent에 숙련자용 압박을 넣지 않는다.
- 동기 제공은 **침묵하는 규칙 구조로만** 이뤄진다. 게이지 관련 UI 문구는 격려도
  훈계도 하지 않는다(RFC 0001 6절 톤 원칙 유지).
...
- RFC 0001의 "긴장감" 프레이밍만 남아, 밸런싱이 압박 장치 방향으로 표류할 수 있다.
...
- 입문 유저에게도 실패가 매우 드물어, "실패 가능"이라는 RFC 0001의 긴장 연출이
  약해질 수 있다.
...
RFC 0001의 Play/Observer 경계, 비행 규칙 명칭, 난이도 축 분리와
RFC 0002의 Breakthrough Perfect-only 회복은 그대로 유지한다.
이 RFC는 비행 규칙의 도입 목적과 Takeoff/Ascent의 밸런싱 방향만 추가로 확정한다.

## 7. 영향 받는 문서

- `docs/rfd/0001-flight-rules-and-observer-boundary.md`
  - 후속 결정으로 이 RFC를 표시한다. Ascent의 Hard Gauge 참조가 구조 참조임을 명확화한다.
- `docs/research/djmax.md`
  - RESPECT V 게이지의 실질적 기능(입문 유저 동기 장치) 관찰을 보강한다.
- `docs/context/glossary.md`, `CONTEXT.md`
  - 비행 규칙 항목에 이 RFC 링크를 추가한다.
- 후속 비행 규칙 구현 스펙 (RFC 0001 §8의 `game-core.md`/`scoring.md` 갱신 포함)
...
2. 입문 유저가 실패했을 때 Observer로의 안내 경로를 어떻게 설계하는가
   (RFC 0001 §9 Q4와 연결).
3. 비행 규칙의 기본값은 가장 관대한 Takeoff인가 (RFC 0001 §9 Q5와 연결).
