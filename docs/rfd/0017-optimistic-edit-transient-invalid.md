...
**탈출구는 특례가 아니라 규칙의 귀결이다 (발화 경계):** 이 게이트는 **차트는 그대로 두고 선택만 바뀌는 전이**에만 발화한다. **undo/redo·위반 노트 삭제**는 차트를 **함께** 바꿔 위반 자체를 없애는 전이라, `removed`에 그 노트가 들어 있어도 통과한다 — 전이 후 그 노트는 더 이상 위반이 아니거나(undo/redo) 아예 존재하지 않는다(삭제). 즉 "갇힘 방지"를 위해 예외를 따로 열거할 필요 없이, 발화 경계 하나가 undo·삭제를 자동으로 열어 준다. 게이트가 봉쇄하는 것은 "위반을 그대로 둔 채 선택만 치워 시야에서 지우는" 전이뿐이다.
...
- 위반 시각화의 언어(색·아이콘·목록 패널). ~~위반이 화면 밖일 때의 안내(미니맵 표시 등)~~ **→ 화면 밖 위반 인지는 §3-5(선택 해제 게이트 + 토스트)로 해소(2026-07-11 개정).** 시각화 언어 자체(색·아이콘·목록 패널)와 토스트 문구는 구현 단계 확정 대상으로 남는다.
