...
**Status:** Accepted (2026-07-09) · 개정 (2026-07-11, §3-5 선택 해제 게이트 추가)
...
- **선택 해제 게이트(§3-5)의 두 구현 선행 조건**: (1) 흩어진 선택 해제 지점을 `SelectionSlice` 단일 관문으로 수렴(전이 diff를 한 곳에서 계산). (2) 게이트는 "`removed` 중 위반에 **관여하는 노트**"를 알아야 하므로 **노트 단위 위반 귀속 맵**과 매 전이 재계산 수단이 필요하다 — 현 `validateChart`는 위반 **메시지 배열**만 반환하므로 귀속 산출이 선행이고, 매 전이 전수 검증 비용은 "스윕 O(N) 순회" 백로그와 맞닿는다. (귀속 산출은 이 RFD의 구현 작업에서 `ValidationError.refs`(노트 index 참조)로 상당 부분 채워졌으므로, 그 구현이 머지되면 이 선행의 큰 몫이 해소된다.)
