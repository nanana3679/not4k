...
**결정: 선택 해제 게이트를 신설한다.** 선택 집합을 비우는 행위(Esc·빈 곳 클릭·다른 노트 클릭으로 선택 교체)를 시도할 때, **해제 대상 노트 중 의미 위반에 관여하는 노트가 하나라도 있으면 해제를 거부**하고, **토스트로 어떤 위반이 왜 남아 있는지** 알린다. 선택은 그대로 유지된다.

**place-then-fix 불변:** 위반 상태로 배치하는 것도, 선택 묶음 안에서 재드래그로 고치는 것도 그대로다(§3-3 (a)). 바뀌는 것은 **"나중에"의 상한**뿐이다 — **선택 해제 시점 = "이 편집 묶음을 마무리한다"는 신호**로 규정하고, 그 순간에 위반을 안은 채 **시야 밖으로 이탈**하는 것만 막는다. "기약 없는 나중에"가 "선택을 푸는 순간까지"로 bounded될 뿐, 위반은 여전히 커밋되고 히스토리에 남는다(하드 거부 아님).

**탈출구(갇힘 방지):** 선택을 못 풀게 하면 "위반을 해소하지 못하는 사용자가 갇히는" 위험이 생기므로, 다음 두 경로는 위반이 남아 있어도 **항상 통과**시킨다 — (a) **undo**(편집 직전으로 복귀 → 위반·선택 동시 해소), (b) **위반 노트 삭제**(위반 원인 제거). 차단 대상은 **순수하게 선택만 비우는 행위**뿐이다. 따라서 사용자는 언제든 undo·삭제로 벗어날 수 있고, 게이트는 "무시하고 떠나기"만 봉쇄한다.

**게이트 층위 (저장 게이트와의 관계):** 이 게이트는 저장·플레이 진입 게이트(§3-2)와 **층위가 다르다**. 저장 게이트는 **차트 전역** 위반을 저장 직전에 최종 강제하는 반면, 선택 해제 게이트는 **선택 집합 국소** 위반을 **편집 흐름 안에서 조기에** 인지·강제한다. 트리거 기준도 전역이 아니라 **"해제하려는 노트 중 위반 관여"**다. 따라서 선택 밖에 남은 화면 밖 위반(예: 선택을 이미 푼 뒤 다른 노트와 겹쳐 생긴 위반)은 이 게이트가 잡지 못하며, 그건 **저장 게이트가 최종 방어선**으로 남는다. 두 게이트는 대체가 아니라 조기(국소)/최종(전역)의 2단 방어다.
