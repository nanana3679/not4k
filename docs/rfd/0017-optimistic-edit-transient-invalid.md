...
- **구조 검증 (structural)** — 위반 시 차트가 **데이터로서 성립하지 않거나(파싱 불가: `endBeat < beat` 역전·유효하지 않은 레인 좌표), 소비자를 크래시시킨다(§3-4의 measure 무한루프처럼 transient로도 견딜 수 없는 것).** 즉 구조 버킷의 판정 기준은 **"데이터 malformed" ∨ "크래시 유발"** 2요소다(둘 중 하나면 구조). **편집 중에도 절대 불가** → `setChart`가 항상 하드 거부한다. (크래시 유발분은 소비자 하드닝으로 크래시를 제거하면 의미로 강등 가능 — 라벨이 아니라 소비자 내성이 경계다.) **RFD 0018 전방 정정:** `lane`은 양의 정수 축이므로 상한 이탈은 구조 위반이 아니다. `lane < 1`·비정수만 `laneMalformed`이다.
...
**구현(2026-07-12, `feat/rfd0016-selection-integration`):** `SelectionSlice.setSelection`에 게이트 구현 — `removed = 현재 − 정규화(다음)`(notes·zones)가 의미 위반에 관여하면 전이 거부 + 토스트(사유 포함, 박스 드래그 연발 대비 1초 스로틀). `clearSelection`도 같은 관문을 지난다. 위반 출처는 `validateChart` refs(`violationsInvolving` 재사용) — 해칭·게이트가 같은 판정을 본다. **RFD 0018 구현(2026-07-31):** 보조 레인 노트도 `chart.notes` 전역 index로 변환되어 같은 게이트를 통과하므로 기존 `removed.extraNotes` 제외 특례는 폐지되었다. **탈출구의 기계화**: `setChart`가 노트·구간 **개수가 줄어드는 커밋이면 선택을 같은 트랜잭션에서 전부 비운다** — 삭제·잘라내기·붙여넣기 취소가 게이트를 지나지 않는 원자 전이가 되는 지점(인덱스 밀림 방지 겸). 이때 `deleteSelected`·`cancelPaste`는 반드시 **차트 커밋 → 선택 해제** 순서여야 한다(역순이면 위반 노트 삭제가 게이트에 막힌다). 알려진 edge(백로그): 위반 노트를 쥔 채 붙여넣으면 차트는 커밋되고 선택 전이만 거부되는 어긋남.
...
- ~~**각 규칙의 구조/의미 잠정 귀속**~~ **→ 감사 완료(§3-4): `timeSigNatural`=구조(크래시), 나머지 10=의미.** (아래는 감사 전 잠정 초안, §3-4가 정본.) `validateChart`는 11개 하위 함수로 구성된다:
  ...
  - **구조: 감사 결과 `timeSigNatural` 1건(§3-4) + 신설 대상.** timeSigNatural(분자/분모 ≤0)은 measure 루프 3곳을 무한루프로 만들어 구조로 확정. 그 외 역전(`endBeat<beat`)·유효하지 않은 레인 좌표는 당시 검사 자체가 없어 **신설 대상**이었다. **RFD 0018 구현 후 정본:** 레인 검사는 `laneMalformed`이며 하한(`lane < 1`)·비정수만 거부하고 상한은 두지 않는다.
