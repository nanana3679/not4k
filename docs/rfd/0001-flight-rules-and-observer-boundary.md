...
플레이 중 생존 지표를 **고도**로 비유하고, 고도에 따라 배경의 수평선,
시야각, 격자 밀도, 오브젝트 크기와 속도가 달라지는 방식이다.

초안의 비행 규칙은 다음과 같다. beatmania IIDX 계열 게이지는 설계 참조 모델이며,
...
고도는 배경 연출 또는 최근 퍼포먼스 시각화로만 사용한다.
...
- 클리어 램프, 고도, 비행 결과 같은 장기 목표를 만들 수 있다.
...
Play에는 비행 규칙을 적용하되, 입력 가능한 무기록 완주 모드를 별도로 제공한다.
...
비행 규칙이 실패를 만들더라도 not4k의 태도는 유지한다.

- 실패를 격려하거나 훈계하지 않는다.
- "추락했습니다", "조금만 더", "포기하지 마세요" 같은 감정 해석형 문구를 쓰지 않는다.
- 결과 화면은 `FAILED`, `CLEAR`, 고도값, 달성률, 판정 분포처럼 사실만 표시한다.
- 실패 연출은 상태 변화의 표시여야 하며, 플레이어를 조롱하거나 과장하지 않는다.
- 중앙 4레인은 항상 안정적으로 유지하고, 고도 연출은 외곽 배경에 제한한다.
...
- 수평선, 격자, 원거리 비콘, 외곽 오브젝트만 고도에 반응한다.
- 높은 고도에서는 수평선이 보이고 시야가 넓으며 오브젝트가 작고 느리다.
- 낮은 고도에서는 수평선이 낮아지고 근거리 격자가 커지며 속도감이 강해진다.
- 위험 상태에서도 레인 폭, 판정선, 리프트, 서든은 흔들지 않는다.
- 고도 변화는 장식이 아니라 실제 비행 규칙 상태와 연결되어야 한다.

현재 코드에는 임시 고도 모델과 고도별 원근 배경 프리셋이 존재한다.
후속 구현은 이 placeholder를 판정 결과 기반 고도 모델로 교체하는 방향이 자연스럽다.
...
- `docs/context/glossary.md`
  - `고도`, `비행 규칙`, `Takeoff`, `Ascent`, `Breakthrough` 정의 추가
...
5. 비행 규칙은 모든 차트에 적용되는가, 차트별로 선택 가능한가?
6. 랭킹과 플레이 기록은 비행 규칙별로 분리할 것인가?
