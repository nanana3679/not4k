...
**Status:** Accepted (2026-07-12) — §8 미결 5건 grilling으로 전량 확정
...
**C + 3-1(A) + 3-3(a) + 3-4(a) + 3-6(a).** 게임·저장은 경계에서 무변경으로 봉인하고, 통합의 수술은 에디터·공유 모델 안에서만 일어난다. §8 미결 5건은 grilling으로 확정(각 항목 참조).
...
5. `extraLaneCount` 축소 시 범위 밖(lane > 4+N) 노트는 **숨김 유지**(§8-4 결정 — 현행 계승): 데이터는 남고 표시만 사라지며, 레인 수를 되돌리면 다시 보인다.
