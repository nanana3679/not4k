...
**Status:** Accepted (2026-07-12) — §8 미결 5건 grilling으로 전량 확정 · 개정 (2026-07-12, 리뷰 반영: `laneAxis` 경계 레이어·라이브 진입 경계·§8 추가 결정 2건)
...
**불변식: `lane > MAIN_LANE_COUNT`를 아는 코드는 이 모듈과 그 직접 소비자(경계 지점)뿐이다.** 나머지 코드는 노트를 레인 무차별로 다룬다.
...
| **테스트플레이/프리뷰** | 에디터 라이브 `chart` 객체를 **직렬화 없이 직접** 전달(`performPlayTest` → `game.setChartData`, `useFileOperations.ts:91`) | **`mainNotes()` 필터 필수.** 현행은 보조가 별도 배열이라 이 경로가 *공짜로* 안전했다 — 통합이 그 암묵 보호를 없애므로 명시 필터로 대체한다. RFD 0017 §3-3의 플레이/프리뷰 진입 게이트와 같은 지점. 필터 후 `totalJudgments`·noteTimes 등 점수 분모가 lane ≤ 4 기준인지 확인(분모 오염 방지) |
| 저장 | `chartAssetPersistence` | `mainNotes()` → 차트 파일, `auxNotes()`+`toAuxIndex` → 보조 파일(현행 포맷 유지) |

미필터 시 실측 피해(리뷰 감사): 보조 포인트 노트 auto-Miss로 콤보 파괴(`JudgmentEngine:546-581`), 보조 롱노트 영구 미판정(`:959-972`), 달성률 분모 오염(`PlayScreen:120-139`), 필드 밖 렌더(`GameNoteRenderer:461`).
...
1. `extraNotes` 소비 **26파일**(에디터 21·shared 5) — 슬라이스 분해 필수. **순서(리뷰 권고 반영)**: ① 모델+`laneAxis`+**게임 라이브 진입 필터 동시 착지**(필터 없이는 슬라이스 ①부터 테스트플레이가 오염됨) → ② **저장 분리+로드 병합을 한 슬라이스로**(왕복 짝 — 쪼개면 저장→재로드가 중간에 깨짐) → ③ 검증 흡수 → ④ 선택/드래그(최대·최고위험: 박스 축 제거/크로싱 제거/moveExtra 흡수로 세분) → ⑤ 렌더/히트테스트 → ⑥ docs.
2. `Lane` 참조 총 26파일 중 **전환 대상 ~18**(에디터·모델 → `number`), **게임 8파일은 `Lane` 유지**. `as Lane` 캐스트 16지점 — 타입만으로 lane>4 유입을 못 잡으므로 게임 경계 필터에 **테스트를 명시**.
3. 직렬화 경계는 1모듈이 아니라 **3사이트**: 저장 분리(`chartAssetPersistence.buildChartAsset`) + **에디터 로드 병합**(`App.tsx:137-154` — legacy-embedded fallback·`maxUsedLane` 자동 확장과 엉킴, 그 보장 계승 필수) + **dirty 스냅샷**(App·EditorToolbar의 별도 `serializeExtraNotes` 비교).
...
- **RFD 0017 충돌 기록**: 0017 §3-4가 "신설 예정"으로 남긴 **레인범위 구조 검사**("존재하지 않는 레인 참조 = malformed")는 본 RFD로 **재정의된다** — `lane > 4`는 더 이상 malformed가 아니고, 상한(`4+extraLaneCount`)은 차트 밖 store 설정(§8-3)이라 **구조 검증(차트만 입력)으로는 판정 불가능**하다. 하한·비정수(`lane < 1`, 소수)만 구조 후보로 남고, 상한 초과는 로드 병합의 `maxUsedLane` 확장과 §8-4 숨김이 담당한다.
...
3. ~~`extraLaneCount`의 소속~~ **→ 현행 store 설정 유지.** 차트 메타로 옮기지 않는다 — 보조 레인 수는 제작 환경 설정이지 차트 사실이 아니다.
4. ~~레인 수 축소 시 범위 밖 보조 노트~~ **→ 숨김 유지(현행 계승).** 데이터 보존, 표시만 숨김 — §6-5에 반영.
5. ~~선택·이동의 레인 연속성~~ **→ 확정.** 메인 4 ↔ 보조 1(lane 5)은 한 칸 이동으로 연속 취급(현행 키보드 변환 UX 계승) — "변환"이 아니라 그냥 레인 이동이 된다. 시각 경계선은 유지.
6. **(리뷰 추가) 붙여넣기의 보조 레인 초과 → D3(자동 확장) 유지 (2026-07-12).** §8-4(숨김)와 충돌이 아니라 **연산별 정책으로 양립**한다: 방금 붙여넣은 노트가 안 보이면 안 되므로 **붙여넣기는 자동 확장**, 사용자가 의도적으로 줄인 **레인 수 축소는 숨김 유지**.
