...
- **(해소) 홀드 중 탭의 탭 키 과발화**: 유지 키가 아닌 다른 키(끝점 헤드/탭)의 릴리즈가 롱의 판정을 가로채던 문제를 **연결(`o-o-`)과 종결(`o-o`) 모두에서** 해소했다.
  - **연결**: `hasImmediateFollowingLongNote`가 끝점에 헤드가 섞여도 뒤따르는 롱을 찾도록 보정(전체 스캔)하고, `tryEndpointJudgmentOnRelease`의 연결 분기를 제거해 **연결은 끝점 update의 held-or-grace에 위임**한다.
  - **종결**: 단독 롱은 **레인이 완전히 떼질 때(heldKeys 비면)만 종결**하도록 게이트한다 — 다른 키가 남아 있으면 그 릴리즈로 종결하지 않으므로 탭 키가 종결을 가로채지 않는다. update 경로의 isHeld/lastRelease 기준과 일치하며 lane-held·이어잡기와 일관.
  - 회귀: "홀드 중 탭(o-o)", "… 레인 완전 릴리즈에 종결", "o-o- 연결 … 강건". (엣지: 유지 키를 탭 키보다 먼저 떼는 비정상 순서에서는 마지막 릴리즈 타이밍을 쓰지만 부당한 손해는 없음 — 적대적 검증 확인.)
