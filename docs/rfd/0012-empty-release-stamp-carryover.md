...
- **(확인 완료) 분할 릴리즈(`D=-`)와의 상호작용**: doubleLong 종결은 도장 가드와 무관하지 **않다** — keyup 종결 분기(`tryEndpointJudgmentOnRelease`의 DOUBLE_LONG 분기)는 `isEmptyRelease` 도장 가드 **뒤에 있어 도장에 막힐 수 있다**(도장 잔류 베이스라인에서 keyup 직후 판정 0건으로 실측). 그럼에도 등급이 보존되는 근거는 **이중화**다: `updateDoubleLongKeyRelease`가 가드보다 **앞에서** 키별 `lastReleaseTimeMs`를 무조건 기록하고, `update` 폴백(`judgeDoubleLongEndpoint`)이 그 시각으로 같은 등급을 재판정한다. 즉 **경로 면역이 아니라 결과 면역**이며(회수·부여 조건화 유무와 무관하게 등급 동일 — 막힌 경우 emit만 끝점 이후 첫 update 프레임으로 지연), 이 면역의 담지자는 폴백의 released-키 재판정이다. **폴백을 "미릴리즈 키 Miss 처리"로 축소 단순화하면 면역이 조용히 깨진다.** hold-only 더블롱 → 갭 → 더블롱 분할 릴리즈 회귀 테스트로 잠금. 이로써 이 RFD의 열린 항목은 없다.
