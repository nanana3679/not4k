...
- **(확인 완료) 분할 릴리즈(`D=-`)와의 상호작용**: doubleLong 종결은 도장 가드와 무관하지 **않다** — keyup 종결 분기(`tryEndpointJudgmentOnRelease`의 DOUBLE_LONG 분기)는 `isEmptyRelease` 도장 가드 **뒤에 있어 도장에 막힐 수 있다**(도장 잔류 베이스라인에서 keyup 직후 판정 0건으로 실측). 그럼에도 등급이 보존되는 근거는 **이중화**다: `updateDoubleLongKeyRelease`가 가드보다 **앞에서** 키별 `lastReleaseTimeMs`를 무조건 기록하고, `update` 폴백(`judgeDoubleLongEndpoint`)이 그 시각으로 같은 등급을 재판정한다. 즉 **경로 면역이 아니라 결과 면역**이며(회수·부여 조건화 유무와 무관하게 등급 동일 — 막힌 경우 emit만 끝점 이후 첫 update 프레임으로 지연). **담지자는 경로에 따라 갈린다(P1 프로브로 실측)**: 정상 동작(회수 on)에선 keyup 종결(`tryEndpoint`)이 **즉시** 판정하고 **폴백은 도달하지 않는다**(released 분기에 throw를 심어도 회수 on에서 안 터지고, off에서만 터짐) → **primary 담지자는 회수**이고, 폴백의 released-키 재판정은 **회수/부여 조건화가 깨졌을 때만 발동하는 defense-in-depth**다. **주의**: 등급-결과 회귀 테스트(`update` 후 확인)는 폴백이 등급을 복구하므로 **회수 회귀를 조용히 가린다**(released 분기를 Miss로 바꾸는 뮤테이션에도 0 실패로 실측). 따라서 두 테스트로 나눠 잠근다 — **타이밍 테스트**(추가 `update` 없이 keyup 직후 2판정 단언, 폴백 지연이면 실패)가 **primary(회수)** 를, **등급 테스트**가 폴백의 **결과 면역**을 못박는다. **폴백을 "미릴리즈 키 Miss 처리"로 축소하면 결과 면역이 깨진다.** 이로써 이 RFD의 열린 항목은 없다.
