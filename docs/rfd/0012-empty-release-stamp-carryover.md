...
- **(확인 완료) 분할 릴리즈(`D=-`)와의 상호작용**: doubleLong 종결은 키별 `lastReleaseTimeMs`로 `update` 패스(`judgeDoubleLongEndpoint`)에서 처리돼 `isEmptyRelease` 도장 가드(400줄)를 **타지 않는다**. 단일 롱의 **lane-broadcast 종결**(아무 키의 release나 종결 → 도장 가드 필요)과 달리, doubleLong은 **키별 종결**이라 애초에 lane-broadcast 누설 위험이 없고 도장에 의존하지도·막히지도 않는다 → **도장 carryover에 구조적으로 면역**(회수·부여 조건화 유무와 무관). hold-only 더블롱 → 갭 → 더블롱 분할 릴리즈 회귀 테스트로 잠금. 이로써 이 RFD의 열린 항목은 없다.
