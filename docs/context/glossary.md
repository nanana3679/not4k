...
not4k의 난이도명은 **`Liftoff → Infiltration → Breakthrough`** 순서다. 한 곡 안의 차트를 구분하며 각 단계의 비행 시나리오·연출에 대응한다. 명칭과 배치는 [RFD 0022](../rfd/0022-flight-difficulty-names-and-visuals.md)에서 채택했으며, [RFD 0026](../rfd/0026-game-flight-backgrounds.md)에 따라 곡 선택·로딩 표시와 인게임 배경에 적용한다. 기존 `EASY / NORMAL / HARD`는 저장 식별자를 유지하며 각각 새 이름으로 표시한다. 기존 `EXPERT`는 별도 이름을 유지하고 돌파 배경을 사용한다. 새 차트의 선택지는 세 공식 이름이다. 원격 데이터 일괄 이행과 패턴 제한은 [PRD §12](../prd.md#12-미정-사항)에서 추적한다. 판정명의 잠정 상태는 이 결정과 별개다.
...
세 단계의 이름·순서·연출 배치는 확정했으며 이름 표시와 배경 연결을 구현했다. 클리어·실패를 결정하는 비행 규칙은 미구현이며, 세부 판정 규칙은 [PRD §12](../prd.md#12-미정-사항)에서 추적한다.
