...
**구현**: `src/editor/stores/selectionSlice.ts`의 `setSelection`(정규화 게이트와 같은 자리 — `clearSelection`도 이 관문을 지난다). `removed.extraNotes`는 게이트 대상이 아니다(엑스트라 위반은 시각화 전용). 삭제·붙여넣기 취소는 차트 축소 커밋(`setChart`)이 선택을 원자적으로 비워 게이트를 지나지 않는다.
