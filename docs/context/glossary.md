...
**저장 게이트와 진짜 대칭 (단일 관문)**: 저장 게이트가 `setChart`(차트 쓰기 단일 수렴점)에서 차트 전역 위반을 막듯, 선택 해제 게이트는 `SelectionSlice`(선택 쓰기 단일 수렴점)에서 선택 집합 국소 위반을 막는다 — 조기(국소)/최종(전역)의 2단 방어. 선행: 흩어진 선택 해제 지점을 `SelectionSlice` 관문으로 수렴시켜야 게이트를 얹을 수 있다. **탈출구는 특례가 아니라 발화 경계의 귀결**: 게이트는 "차트 불변·선택만 바뀌는 전이"에만 발화하므로, undo/redo·위반 노트 삭제(차트를 함께 바꿔 위반을 없앰)는 `removed`가 있어도 통과한다. **구조 위반**(`timeSigNatural` 등)은 `setChart`가 항상 거부(§3-2)해 애초에 커밋 불가라 해제 시점엔 의미 위반만 남는다. 선택에 든 적 없는 화면 밖 위반은 이 게이트가 못 잡고 저장 게이트가 최종 방어선이며, §7의 미니맵 지시자는 이와 보완 관계(병행 가능). 상세는 [RFD 0017 §3-5](../rfd/0017-optimistic-edit-transient-invalid.md).
