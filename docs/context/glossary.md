...
선택 집합을 비우려는 행위(Esc·빈 곳 클릭·다른 노트 클릭으로 선택 교체)를 가로채, **해제 대상 노트 중 의미 위반에 관여하는 노트가 있으면 해제를 거부**하고 토스트로 사유를 알리는 게이트다. 낙관적 편집(RFD 0017)에서 위반 시각화(해칭)는 뷰포트 안에서만 보이므로, 위반 노트가 화면 밖으로 벗어나 인지되지 못한 채 방치되는 것을 막는다 — **"선택 해제 = 편집 묶음 마무리" 신호**로 규정하고 그 순간에 위반을 안은 채 이탈하는 것만 봉쇄한다(하드 거부 아님, place-then-fix 유지).

**저장 게이트와 같은 패턴, 스코프만 다르다**: 저장 게이트는 **차트 전역** 위반으로 저장·플레이 진입을 막고, 선택 해제 게이트는 **선택 집합 국소** 위반으로 선택 비우기를 막는다 — 조기(국소)/최종(전역)의 2단 방어. **탈출구**: undo·위반 노트 삭제는 위반이 남아 있어도 항상 통과시켜 "갇힘"을 방지한다. 차단 대상은 순수 선택 비우기뿐이다. 상세는 [RFD 0017 §3-5](../rfd/0017-optimistic-edit-transient-invalid.md).
