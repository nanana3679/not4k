...
1. 개발 환경에서는 `pnpm dev`로 서버를 실행하고 `/lab/judgment-playtest`를 연다. 공개 환경에서는 [Lab 카탈로그](https://nanana3679.github.io/not4k/lab/)에서 판정 실플레이를 선택한다. Lab은 게임 앱의 production 번들과 라우트에서 제외되며, `pnpm run build:lab`으로 별도 공개 빌드한다. 경로·등록 규칙은 [Lab 미리보기 지침](../agents/lab-previews.md)을 따른다.
...
에디터와 공유하는 독립 모듈 `src/editor/playtest/performPlayTest.ts`로 차트를 검증하고 새 차트·클릭트랙·시작 시각·Lab 복귀 URL을 설정한다. 서버의 곡이나 오디오 에셋 없이 실행할 수 있다.
