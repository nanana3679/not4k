...
기존 4키 게임에서 유저는 레인을 2/2(왼손 2레인, 오른손 2레인)로 고정 분할하여 인식한다. not4k에서는 트릴 앵커가 한 손을 전담시키고, 건너가기를 통해 반대 손이 내측 레인까지 담당하면 1/3, 3/1 같은 비대칭 분할이 가능해진다. 이는 차트 디자이너에게 완전히 새로운 설계 축을 제공한다.
...
- 차트 디자이너는 트릴 속도와 반대 손 밀도를 반비례시킴으로써 전체 난이도를 일정하게 유지하거나, 동비례시킴으로써 난이도 스파이크를 만들 수 있다.
...
### 3. PLv.와 차트 Lv.를 일치시킨다

초급(Lv.4~6) 차트에 가변 분할 전환(PLv.11)이 등장하면 안 된다. 각 피스의 PLv.가 차트의 Lv.를 초과해서는 안 된다. 피스별 PLv.는 위 매트릭스 및 `piece-definition.md`의 PP-000 체계를 기준으로 한다.
