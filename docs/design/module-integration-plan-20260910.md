...
2026-09-10. **8종 개별 시연과 기존 절단면 광원 속 A~H 선택 비교를 구현했다.** E 하나의 통합에 이어8종을 같은 고도·거리에서 바꿔보는 기능을 추가했다. E의 주변 원경·뒤 골조·하부 연결에 이어, 나머지7종도 원본 고정부에서 y=−640까지 이어지는 구조 연장을 추가했다. 이후8종의 연결부를 관제실·정비층·환승층·적재층 등 용도가 있는 수직 시설로 재구성했다. 긴 빈 축에 무늬만 추가하지 않고, 실제 방과 열린 작업 공간을 층 묶음으로 붙인다. `구조 연장`은 A~H 공통 선택이며 E의 입체 주변 건축은 별도로 유지한다. 다수 건물 최적화·본편 통합은 아직 설계 단계다. 원래 시연의 기본 화면과 RFD 0025, 본 게임의 규칙은 유지한다. 구현 검증은 [8종 시연 기록](../../output/prototypes/module-fleet-20260910/NOTES.md), [E+광원 통합 비교 기록](../../output/prototypes/breakthrough-hangar-integration-20260910/NOTES.md)에 둔다. 미술 검수는 사용자가 하며, 제품 상태·최종 연출의 미정 사항 추적은 [PRD §12](../prd.md#12-미정-사항)를 따른다.

후속으로 **비행 공간부터 설계한 물류 시설 하나**를 같은 비교 화면의 `study=passage`에 구현했다. 중앙의 실제 빈 경로를 먼저 확보하고, 고도별로 좁은 적재동 사이·지붕 위·연결 교량 아래를 연속 통과한다. 기존 A~H 전체를 교체하는 결정은 아니며, 한 시설의 접근→진입→내부→출구 경험을 사용자에게 검수받는 단계다. [시설의 범위와 검증](../../output/prototypes/breakthrough-hangar-integration-20260910/NOTES.md#비행-공간부터-설계한-시설-하나-2026-09-10)을 참조한다.
...
위 표는 원본 단독 모델의 측정이다. 후속 사용층·수직 시설을 켠 통합 시연의 호출 수는 더 크며, 최신 관찰값은 [통합 시연 기록](../../output/prototypes/breakthrough-hangar-integration-20260910/NOTES.md#용도-있는-수직-시설-2026-09-10)에 따로 둔다. 이 결과는 건물 내부 병합과 윤곽선 묶음을 먼저 검토할 근거다. 호출 수를 줄였을 때 얼마나 빨라지는지는 목표 기기에서 별도 측정해야 한다.
