...
try{
 ...
 await page.locator('[data-module="A"]').click();await settle();const a=await snap();check('A 선택은 E 주변만 숨기고 A 구조 연장과 공통 조절을 유지',a.module==='A'&&!a.space.farVisible&&!a.space.stationVisible&&a.space.extensionVisible&&a.space.extensionModule==='A'&&await page.locator('#surroundings').isDisabled()&&!await page.locator('#extensions').isDisabled());
