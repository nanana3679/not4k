...
function scene(altitude: number, width = 1280, height = 720, clearance = 1) {
  const settings = readSettings(`altitude=${altitude}&clearance=${clearance}`);
    ...
    for (const clearance of [0, 1]) {
      for (const altitude of [0, 1]) {
        ...
        it(`${width}×${height}·근접${clearance}·고도${altitude * 100}%의 모든 직선은 ${direction}를 향한다`, () => {
          const { settings, pyramid, view, lines } = scene(altitude, width, height, clearance);
