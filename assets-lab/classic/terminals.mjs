import { referenceBodySvg } from './terminal-material.mjs';
...
export function startTerminalSvg(body, flavor) {
  ...
  const reference = referenceBodySvg(body, flavor);
  const referenceDefs = reference.match(/<defs>([\s\S]*?)<\/defs>/)[1];
  ...
  // Retain the exact body colors at the open edge. The sampled material takes
  // over within the cap; keeping the outer rails outside this mask also
  // preserves their uninterrupted join. The reference scene is opaque, so
  // this blends colors without reducing the terminal's alpha.
  let referenceSurface = shapeSurface(reference);
  const reflectedMetal = [47,907].map(x => shapeBodyRect(`<rect x="${x}" y="0" width="46" height="200" fill="url(#finish-metal-tint)"/>`,double)).join('');
  referenceSurface = referenceSurface.replace(/(<g\b[^>]*data-layer="emission"[^>]*>)/,
    `<g data-layer="finish-color-reflection">${reflectedMetal}</g>$1`);
  referenceSurface = referenceSurface.replace('<g data-light="spill">', `<g data-light="spill"><g clip-path="url(#finish-halo-area)"><path d="${core}" fill="${accent}" opacity=".58" filter="url(#finish-halo)"/></g>`);
  surface += `<g data-material="reference-04" mask="url(#finish-reference-blend)" clip-path="url(#finish-reference-interior)">${referenceSurface}</g>`;
...
<title>${label}</title>
<defs>${bodyDefs}${referenceDefs}
<linearGradient id="finish-reference-mix" gradientUnits="userSpaceOnUse" x1="0" x2="0" y1="0" y2="70"><stop offset="0" stop-color="#ffffff" stop-opacity="0"/><stop offset=".1" stop-color="#ffffff" stop-opacity="0"/><stop offset=".57" stop-color="#ffffff" stop-opacity=".65"/><stop offset="1" stop-color="#ffffff"/></linearGradient>
<linearGradient id="finish-metal-tint" gradientUnits="userSpaceOnUse" x1="0" x2="0" y1="42" y2="192"><stop offset="0" stop-color="#245a8f" stop-opacity="0"/><stop offset=".5" stop-color="#326b9c" stop-opacity=".25"/><stop offset="1" stop-color="#215887" stop-opacity=".58"/></linearGradient>
<mask id="finish-reference-blend" maskUnits="userSpaceOnUse" x="0" y="0" width="1000" height="200" style="mask-type:alpha"><rect x="0" y="0" width="1000" height="200" fill="url(#finish-reference-mix)"/></mask>
<clipPath id="finish-reference-interior" clipPathUnits="userSpaceOnUse"><rect x="47" y="0" width="906" height="194"/></clipPath>
...
<filter id="finish-halo" filterUnits="userSpaceOnUse" x="300" y="${haloTop - 9}" width="400" height="${209 - haloTop}"><feGaussianBlur stdDeviation="18"/></filter>
</defs>
${surface}
</svg>\n`;
