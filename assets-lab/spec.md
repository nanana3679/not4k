...
롱노트를 놓쳤거나 바디 유지에 실패한 경우, 해당 롱노트 전체가 **무채색 계열**로 전환되어 실패 상태를 즉시 전달한다. 싱글 롱노트, 더블 롱노트, 트릴 롱노트 모두 동일하게 적용된다. 판정 메커니즘은 `note-system.md`의 "바디 판정"을 참고한다.
...
| 노트가 위치한 레인 | 입력 상태 | dimLeft | dimRight | 시각적 결과 |
|-----------|-----------|---------|----------|-------------|
