# Gameplay

Gameplay 컨텍스트는 유저가 차트를 선택해 플레이하고, 입력이 판정과 결과로 이어지는 경험을 다룬다.
노트 타입, 피스, 난이도 용어는 먼저 루트 `CONTEXT.md`의 Core domain을 따른다.

## Language

**플레이 세션**:
유저가 하나의 차트를 시작해서 결과 화면에 도달하기까지의 한 번의 플레이이다.
_Avoid_: 게임 실행

**Observer 계열**:
실패 조건 없이 차트를 관찰하고 손배치를 계획하는 흐름이다. 완주, 관찰, 구간 반복 학습은 Play가 아니라 Observer 계열이 담당한다.
_Avoid_: 쉬운 플레이

**곡 선택**:
유저가 플레이할 곡과 차트를 고르는 흐름이다.
_Avoid_: 파일 선택

**플레이 화면**:
차트가 재생되고 유저 입력이 판정되는 화면이다.
_Avoid_: 게임 화면

**결과 화면**:
플레이 세션이 끝난 뒤 판정 집계, 달성률, 랭크, 풀콤보 여부, 비행 규칙의 클리어/실패 여부를 보여주는 화면이다.
_Avoid_: 스코어 화면

**입력-노트 매칭**:
유저 입력을 판정 윈도우 안의 노트에 연결하는 판정 전 단계이다.
_Avoid_: 키 이벤트 처리

**가장 이른 노트 매칭**:
판정 윈도우 안에 여러 노트가 있을 때 시간상 가장 앞선 노트가 입력을 먼저 받는 규칙이다.
_Avoid_: 가장 가까운 노트 매칭

**빈 레인 입력**:
시각적으로 비어 보이는 레인에 입력했지만 판정 윈도우 안의 노트와 연결될 수 있는 입력이다.
_Avoid_: 무시 입력

**처리된 노트**:
판정이 끝나 더 이상 플레이어가 읽거나 입력할 필요가 없는 노트이다.
_Avoid_: 지나간 노트

**콤보**:
Bad 또는 Miss 없이 이어진 연속 판정 수이다.
_Avoid_: 연속 입력 수

**풀콤보**:
Bad 또는 Miss 없이 플레이 세션을 끝낸 결과 상태이다.
_Avoid_: 올콤보

**달성률**:
차트에서 얻을 수 있는 총점 대비 실제 획득 점수의 비율이다.
_Avoid_: 정확도

**랭크**:
달성률을 구간별로 요약한 결과 등급이다.
_Avoid_: 난이도 등급

**오디오 오프셋**:
출력 장치의 소리 지연을 보정하기 위해 음악 재생 기준을 이동시키는 설정이다.
_Avoid_: 판정 오프셋

**입력 오프셋**:
입력 장치 지연이나 개인 체감 타이밍을 보정하기 위해 입력 기준을 이동시키는 설정이다.
_Avoid_: 오디오 오프셋

**리프트**:
판정선 위치를 위로 올려 유저의 시야 중심을 조정하는 플레이 설정이다.
_Avoid_: 스크롤 속도

**서든**:
레인 상단 일부를 가려 노트가 보이는 시간을 줄이는 플레이 설정이다.
_Avoid_: 리프트

**게이지**:
플레이 중 실패 여부를 결정하는 생존 지표이다.
_Avoid_: 달성률

**비행 규칙**:
Play에서 고도 기반 클리어/실패를 결정하는 규칙이다. 현재 비행 규칙은 Takeoff, Ascent, Breakthrough이며, UI와 코드는 영어 명칭을 사용한다. 결정 배경은 `../../docs/rfd/0001-flight-rules-and-observer-boundary.md`를 따른다. beatmania IIDX 참조 모델은 `../../docs/research/beatmania.md`의 게이지 옵션 보충 자료를 따른다.
_Avoid_: 난이도명

**고도**:
비행 규칙의 현재 상태를 표현하는 플레이 중 지표이다. 높을수록 안정 비행, 낮을수록 실패 위험을 의미한다.
_Avoid_: 달성률

## Relationships

- **플레이 세션**은 하나의 **차트**와 하나의 **비행 규칙**을 대상으로 진행된다.
- **Observer 계열**은 실패 조건 없이 차트를 관찰하고 학습하는 흐름이며, 기록 대상 Play와 구분된다.
- **곡 선택**은 **플레이 세션** 전에 곡과 차트를 결정한다.
- **플레이 화면**은 입력을 **입력-노트 매칭**으로 연결하고 판정을 만든다.
- **가장 이른 노트 매칭**은 노트 배치 순서와 판정 순서를 일치시킨다.
- **처리된 노트**는 읽기 부담을 줄이기 위해 플레이 화면에서 제거된다.
- **결과 화면**은 **달성률**, **랭크**, **콤보**, **풀콤보**, **비행 규칙** 결과를 보여준다.
- Play는 **비행 규칙**에 따라 실패할 수 있으며, 실패 조건 없는 완주와 관찰은 **Observer 계열**이 담당한다.
- **고도**는 **게이지**의 비행 콘셉트 표현이며, **달성률**이나 **랭크**와 구분한다.
- **오디오 오프셋**과 **입력 오프셋**은 서로 다른 지연 원인을 보정한다.
- **리프트**와 **서든**은 판정 규칙이 아니라 시야 조절 설정이다.

## Example dialogue

> **Dev:** "빈 레인에서 누른 입력은 그냥 버리면 되나요?"
> **Domain expert:** "판정 윈도우 안에 노트가 있으면 **빈 레인 입력**으로 Bad가 날 수 있어요. 무조건 무시하면 안 됩니다."
>
> **Dev:** "끝까지 쳐보고 싶은데 Play에서 비행 규칙 실패가 나면 어떻게 하나요?"
> **Domain expert:** "완주와 관찰은 **Observer 계열**이 담당합니다. Play는 **비행 규칙**에 따른 클리어/실패가 있는 기록 대상 흐름입니다."

## Flagged ambiguities

- **랭크**는 결과 등급이고 **난이도 등급**은 차트 레벨 구간이다.
- **오디오 오프셋**과 **입력 오프셋**은 합치지 않는다.
- **비행 규칙**은 **난이도명**, **Lv.**, **난이도 등급**과 바꿔 쓰지 않는다.
- **Breakthrough**는 EX-HARD 자체의 복제가 아니라 TIME HELL mode의 회복 불가 동작을 참조한다.
- **고도**는 **달성률**이나 **랭크**와 다르다.
